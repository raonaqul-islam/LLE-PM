% MONTE-CARLO SIMULATION FOR FEEDBACK MODULATION WITH THERMOREFRACTIVE
% NOISE
clear
close all
...
parfor k = 1:N_sim
    ...
    for ix = 1:N_steps        
        ...
        A_tilde     = -(1+1i*alpha) + 1i*alphaM*mu + 1i*Dint_T;
        ...
        circmean(ix)   = angle(z);                           
        if ix > 4*skip
            dw_rep(ix) = (theta0(ix)-theta0(ix-1))/dt;       
        end 
    end    
    % Save data
    data_structure = struct('circmean',circmean);    
    save(sprintf('./data/fbackpmTRN030126/fbackpmTRN030126_%d.mat',k),'-fromstruct',data_structure);
end
