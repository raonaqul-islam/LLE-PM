...
% This version starts from zero initial condition and shows that single
% soliton is formed from a cold cavity.
% -----------------------------+
% Raonaqul Islam, UMBC         |
...
savesol = 0;                              % 1 if you want to save the final solution
...
% Initial condition
psi_0 = 0*sech(theta);                     % Zero initial condition (cold cavity)
...
for ix = 1:N_steps  
    ...
    if rem(ix,1/dt) == 0
        ...
        plot(theta/pi,abs(psi_out).^2,'LineWidth',2.5,'Color','b');
        xlabel('\theta/\pi')
        ylabel('|\psi|^2')
